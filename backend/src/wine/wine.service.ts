...
import { IVivino } from 'src/vivino/dto/vivino.dto';
...
export class WineService {
  ...
  getWineRating(name: string): Observable<IWineRating> {
    return this.vivinoService
      .searchWine(name)
      .pipe(map((vivinoList) => this.getVivinoRating(vivinoList)));
  }

  private getVivinoRating(vivinoList: IVivino[]): IWineRating {
    ...
    return ratings.length > 0 ? ratings[0] : undefined;
  }
}
