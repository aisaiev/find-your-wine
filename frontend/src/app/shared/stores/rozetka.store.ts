...
function getWineListItems(): Element[] {
  return [...document.querySelectorAll('li[class~="catalog-grid__cell"]')];
}
...
function getWineName(wineTitle: string): string {
  if (!wineTitle) return undefined;

  wineTitle = wineTitle.toLowerCase();

  if (wineTitle.includes('набор') ||  wineTitle.includes('набір')) return undefined;
  ...
  return wineName ? wineName.join(' ') : undefined;
}
